...
//	ArrayList<NhaCungCap> dsnhacungcap=nhacungcapDao.getInstance().selectAll();
	...
	public XuatHang() {
		...
		table.setModel(
				new DefaultTableModel(new Object[][] {}, new String[] { "Mã máy", "Tên máy", "Số lượng", "Đơn giá" }));
		model = (DefaultTableModel) table.getModel();
		for (Laptop sp : sanpham) {
			model.addRow(new Object[] { sp.getMaLaptop(), sp.getTenLaptop(), sp.getSoLuong(), sp.getGia() });
		}
		...
		btn_suasoluong = new JButton("Sửa số lượng", source.icon_pencil);
		...
		btn_xoasp = new JButton("Xóa sản phẩm", source.icon_remove);
	...
	public void themhang() {
		if (txt_soluong.getText().equals(0))
			JOptionPane.showMessageDialog(null, "Bạn chưa nhập số lượng cần XUẤT");
		else {
			sohang++;
//				long sl=Integer.parseInt(txt_soluong.getText());
			...
			model1.addRow(new Object[] { mm, tm, soluong, dongia });
	...
	public void xoasanpham() {
		...
		if (luachon == JOptionPane.YES_OPTION) {
			tongtien -= Double.parseDouble(model1.getValueAt(t, 2) + "")
					* Double.parseDouble(model1.getValueAt(t, 3) + "");
			jl_tongtien.setText(tongtien + "");
			model1.removeRow(t);
		}
