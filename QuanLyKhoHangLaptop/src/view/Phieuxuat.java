...
//		        jt_tienden.setColumns(10);
		...
		table.setModel(new DefaultTableModel(new Object[][] {},
				new String[] { "Mã phiếu", "Mã cửa hàng", "Số lượng", "Tổng tiền", "Ngày xuất", "Mã nhân viên"
//							"Mã phiếu","Tên nhà cung cấp","Tên nhân viên nhập","Thời gian","Số lượng sản phẩm","Tổng tiền"
		...
		for (PhieuXuat sp : dspn) {
			...
			model.addRow(new Object[] { sp.getMaPhieuXuat(), sp.getMaCuaHang(), sp.getTongSoLuong(), formattedNumber2,
					sp.getNgayXuat(), sp.getMaNhanVien() });
		}
	...
	public void thaydoicombo2() {
		...
		if (cmb_ncc.getSelectedItem().equals("Tất cả")) {
			for (PhieuXuat sp : dspn) {
				...
				model.addRow(new Object[] { sp.getMaPhieuXuat(), sp.getMaCuaHang(), fom2, sp.getNgayXuat(),
						sp.getMaNhanVien() });
			}
		} else {
	...
	public void thanhtimkiem() {
		...
		if (text.equals("")) {
			for (PhieuXuat sp : dspn) {
				DecimalFormat df10 = new DecimalFormat("#");
				String formattedNumber2 = df10.format(sp.getTongTien());
				model.addRow(new Object[] { sp.getMaPhieuXuat(), sp.getMaCuaHang(), sp.getTongSoLuong(),
						formattedNumber2, sp.getNgayXuat(), sp.getMaNhanVien() });
			}
		} else {
			if (timitem.equals("Tất cả")) {
				for (PhieuXuat sp : dspn) {
					if (sp.getMaPhieuXuat().equals(text) && sp.getMaCuaHang().equals(text)
							&& sp.getNgayXuat().equals(text) && sp.getMaNhanVien().equals(text)) {
						DecimalFormat df10 = new DecimalFormat("#");
						String formattedNumber2 = df10.format(sp.getTongTien());
						model.addRow(new Object[] { sp.getMaPhieuXuat(), sp.getMaCuaHang(), sp.getTongSoLuong(),
								formattedNumber2, sp.getNgayXuat(), sp.getMaNhanVien() });
