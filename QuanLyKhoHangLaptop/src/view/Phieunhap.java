...
//		        jt_tienden.setColumns(10);
		...
		table.setModel(new DefaultTableModel(new Object[][] {}, new String[] { "Mã phiếu", "Tên nhà cung cấp",
				"Tên nhân viên nhập", "Thời gian", "Số lượng sản phẩm", "Tổng tiền" }));

		model = (DefaultTableModel) table.getModel();
		for (PhieuNhap sp : dspn) {
			DecimalFormat df7 = new DecimalFormat("#");
			String formattedNumber7 = df7.format(sp.getTongTien());
			model.addRow(new Object[] { sp.getMaPhieuNhap(),
					PhieuNhapDao.getInstance().laytennhacungcap(sp.getMaNhaCungCap()),
					NhanVienDAO.getintance().laytennhanvien(sp.getMaNhanVien()), sp.getNgayNhap(), sp.getTongSoLuong(),
					formattedNumber7 });
		}
	...
	public void thaydoicombo2() {
		...
		if (cmb_ncc.getSelectedItem().equals("Tất cả")) {
			for (PhieuNhap sp : dspn) {
				...
				model.addRow(new Object[] { sp.getMaPhieuNhap(),
						PhieuNhapDao.getInstance().laytennhacungcap(sp.getMaNhaCungCap()),
						NhanVienDAO.getintance().laytennhanvien(sp.getMaNhanVien()), sp.getNgayNhap(),
						sp.getTongSoLuong(), formattedNumber7 });
			}
		} else {
			...
			for (PhieuNhap sp : dspn) {
				if (sp.getMaNhaCungCap().equals(text)) {
					...
					model.addRow(new Object[] { sp.getMaPhieuNhap(),
							PhieuNhapDao.getInstance().laytennhacungcap(sp.getMaNhaCungCap()),
							NhanVienDAO.getintance().laytennhanvien(sp.getMaNhanVien()), sp.getNgayNhap(),
							sp.getTongSoLuong(), formattedNumber2 });
				}
			}
		}
	}
		...
		if (text.equals("")) {
			for (PhieuNhap sp : dspn) {
				...
				model.addRow(new Object[] { sp.getMaPhieuNhap(),
						PhieuNhapDao.getInstance().laytennhacungcap(sp.getMaNhaCungCap()),
						NhanVienDAO.getintance().laytennhanvien(sp.getMaNhanVien()), sp.getNgayNhap(),
						sp.getTongSoLuong(), formattedNumber8 });
			}
		} else {
