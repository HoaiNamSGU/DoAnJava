...
//		scrollPane.setBounds(0, 0, 1189, 602);
        ...
        JLabel manhacc = new JLabel("Cửa hàng");
        manhacc.setFont(new Font("Tahoma", Font.PLAIN, 15));
        panel_bentren.add(manhacc);
         jt_manhacc = new JComboBox();
//         jt_manhacc.setEnabled(false);
         for(CuaHang chtmp : ch) {
        	 jt_manhacc.addItem(chtmp.getMaCH());
         }
...
//    		 PhieuNhapDao.getInstance().deletetheoma(mp);
	...
	public void nhaphang() {
        ...
        String manhacc = (String) jt_manhacc.getSelectedItem();
...
//	 	public PhieuXuat(String maPhieuXuat, String maCuaHang, String maNhanVien, Date ngayXuat, double tongTien, int isDelete) {
		PhieuXuat pn1 = new PhieuXuat(maphieun, manhacc, manhanvi, date, Double.parseDouble(formattedNumber2),tongsoluongphieu, 0);
		PhieuXuatDao.getInstance().insert(pn1);
		for (int i = 0; i < table1.getRowCount(); i++) {
			String masanpham = model1.getValueAt(i, 0) + "";
			int soluongm = Integer.parseInt(String.valueOf(model1.getValueAt(i, 10)));
...
//				public ChiTietPhieuXuat(String maPhieuXuat, String maLaptop, int soLuong, Double thanhTien, int isDelete) {
			...
			LaptopDAO.getintance().update1dulieu1(masanpham, soluongm);
		}
		tongsoluongphieu=0;
		model.setRowCount(0);
...
//					"STT","Mã sản phẩm","Tên sản phẩm","CPU","GPU","Ram","Rom","Hệ điều hành","Màn hình","Hãng","Gía","Số lượng"
						lt.getMaLaptop(), lt.getTenLaptop(), lt.getCPU(), lt.getGPU(),
