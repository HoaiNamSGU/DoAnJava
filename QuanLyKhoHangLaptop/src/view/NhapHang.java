...
//
//			model.addRow(new Object[] { stt, sp.getMaPhieuNhap(),
//					PhieuNhapDao.getInstance().laytennhacungcap(sp.getMaNhaCungCap()),
//					PhieuNhapDao.getInstance().laytennhanvien(sp.getMaNhanVien()), sp.getNgayNhap(),
//					sp.getTongTien() });
//		}
...
//    		 PhieuNhapDao.getInstance().deletetheoma(mp);
	...
	public void nhaphang() {
		...
		PhieuNhap pn1 = new PhieuNhap(maphieun, manhacc, manhanvi, Double.parseDouble(formattedNumber2), tongsl, date,
				0);
		...
		for (int i = 0; i < table1.getRowCount(); i++) {
			...
			ChiTietPhieuNhap ctpn = new ChiTietPhieuNhap(maphieun, masanpham, soluongm, Double.parseDouble(thanhtienm),
					0);
	...
	public void themvaobang() {
		// Kiểm tra xem trường số lượng có rỗng không
		if (!jt_soluong.getText().isEmpty()) {
			// Tăng biến stt1 lên 1
			stt1++;
			// Lấy thông tin laptop từ database dựa trên mã sản phẩm
			Laptop lt = LaptopDAO.getintance().selectById(jt_masanpham.getText());
			// Thêm một hàng mới vào bảng với các thông tin của laptop và số lượng nhập vào
			model1.addRow(new Object[] { stt1, lt.getMaNhaCungCap(), lt.getMaLaptop(), lt.getTenLaptop(), lt.getCPU(),
					lt.getGPU(), lt.getRam(), lt.getRom(), lt.getHeDieuHanh(), lt.getManHinh(), lt.getHang(),
			...
			// Xóa các trường nhập liệu và kích hoạt các nút sửa và xóa sản phẩm
			...
			// Hiển thị thông báo nếu trường số lượng rỗng
