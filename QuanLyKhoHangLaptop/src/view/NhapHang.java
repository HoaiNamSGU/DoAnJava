...
//			 panel_phai.setLayout(new BorderLayout(0, 0));
			...
			table1.setModel(new DefaultTableModel(
				...
				new String[] {
						"Mã sản phẩm","Tên sản phẩm","CPU","GPU","Ram","Rom","Hệ điều hành","Màn hình","Hãng","Gía","Số lượng"
				}
			));
...
//			}
	...
	public void suasanphamthem() {
		 ...
		 if(i<0) JOptionPane.showConfirmDialog(null, "Bạn chưa chọn dòng sửa","THÔNG BÁO",JOptionPane.CLOSED_OPTION);	
		 else {
				 if(sua!=0) {
		   	    	 model1.setValueAt(jt_soluong.getText(), i, 10);
		   	    	 model1.setValueAt(jt_thanhtien.getText(), i, 9);
			   	   	  tongtien+= Double.parseDouble(model1.getValueAt(i, 9)+"");
			   	   DecimalFormat ome= new DecimalFormat("#");
				     String omee = ome.format(tongtien);
			   	   kqtongtiennn.setText(omee+"");
	...
	public void xoasanphamthem() {
    	 ...
    	 if(luachon==JOptionPane.YES_OPTION) {
 			tongsoluongphieu-=Integer.parseInt(model1.getValueAt(i, 10)+"");
	...
	public void nhaphang() {
		...
		DecimalFormat df2 = new DecimalFormat("#");
	     String formattedNumber2 = df2.format(Double.parseDouble(tongtienn));
			PhieuNhap pn1=new PhieuNhap(maphieun,manhacc,manhanvi,Double.parseDouble(formattedNumber2),tongsoluongphieu,date,0);
...
//				 String mncccheck=model1.getValueAt(0, 1)+"";
				 ...
				 if(!lt2.getMaNhaCungCap().equals(lt1.getMaNhaCungCap())) JOptionPane.showConfirmDialog(null, "Bạn chỉ được nhập 1 nhà cung cấp","THÔNG BÁO",JOptionPane.CLOSED_OPTION);	
				 else {
					 lt=LaptopDAO.getintance().lay1sp(jt_masanpham.getText());
						stt1++;
					tongsoluongphieu+=Integer.parseInt(jt_soluong.getText());
					    model1.addRow(new Object[] {
					    		lt.getMaLaptop(),lt.getTenLaptop(),lt.getCPU(),lt.getGPU(),lt.getRam(),lt.getRom(),lt.getHeDieuHanh(),lt.getManHinh(),lt.getHang(),jt_thanhtien.getText(),jt_soluong.getText()
				 ...
				 lt=LaptopDAO.getintance().lay1sp(jt_masanpham.getText());
				 jt_manhacc.setText(lt.getMaNhaCungCap());
		tongsoluongphieu+=Integer.parseInt(jt_soluong.getText());
		    model1.addRow(new Object[] {
		    		lt.getMaLaptop(),lt.getTenLaptop(),lt.getCPU(),lt.getGPU(),lt.getRam(),lt.getRom(),lt.getHeDieuHanh(),lt.getManHinh(),lt.getHang(),jt_thanhtien.getText(),jt_soluong.getText()
